...
Whisper now uses a tiny model for live, revisable drafts about every 0.25 seconds while the primary user speaks, with a faster word-by-word visual reveal. The existing small model remains responsible for the final, higher-accuracy pass over the full utterance. If the user starts speaking again while that pass is running, the previous transcript keeps running and the new speech is captured as the next session. For terms such as ticket names or station names, add an initial prompt:
